...
function escrowState(status) {
    ...
    if (["payment_confirmed", "processing", "ready_for_pickup"].includes(status))
        return "secured_in_escrow";
    if (status === "completed")
        return "released_to_wallet";
    return status;
}
...
export async function getMyBuyerOrders(req, res) {
    const rows = await db
...
export async function getMyFarmerOrders(req, res) {
    const rows = await db
...
const FARMER_STATUS_TRANSITIONS = {
...
function formatBuyerOrder(order, listing, farmer) {
    return {
        order_id: order.orderId,
        order_ref: `#${order.orderRef}`,
