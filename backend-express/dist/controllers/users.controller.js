...
export async function updateUser(req, res) {
    ...
    if (data.locationState !== undefined)
        updates["locationState"] = data.locationState;
    if (data.locationLga !== undefined)
        updates["locationLga"] = data.locationLga;
    if (data.locationVillage !== undefined)
        updates["locationVillage"] = data.locationVillage;
    if (data.bankName !== undefined)
        updates["bankName"] = data.bankName;
    if (data.bankAccountNumber !== undefined)
        updates["bankAccountNumber"] = data.bankAccountNumber;
    if (data.bankAccountName !== undefined)
        updates["bankAccountName"] = data.bankAccountName;
    if (data.preferredLanguage !== undefined)
        updates["preferredLanguage"] = data.preferredLanguage;
    if (data.farmName !== undefined)
...
export async function uploadNinDocument(req, res) {
    const file = req.file;
    if (!file) {
...
export async function uploadOwnershipDocument(req, res) {
    const file = req.file;
    if (!file) {
        res.status(400).json({ error: "No file provided" });
        return;
