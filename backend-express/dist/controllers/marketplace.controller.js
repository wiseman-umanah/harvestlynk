...
export async function getAllListings(req, res) {
    ...
    const rows = await db
        ...
        .where(and(eq(listings.status, "active"), category ? eq(listings.category, category) : undefined, search
        ? or(ilike(listings.productName, `%${search}%`), ilike(listings.category, `%${search}%`))
        : undefined))
...
export async function updateListing(req, res) {
    ...
    if (existing.farmerId !== req.user.userId) {
    ...
    if (d.product_name !== undefined)
        updates["productName"] = d.product_name;
    if (d.category !== undefined)
        updates["category"] = d.category;
    if (d.unit !== undefined)
        updates["unit"] = d.unit;
    if (d.location_state !== undefined)
        updates["locationState"] = d.location_state;
    if (d.location_lga !== undefined)
        updates["locationLga"] = d.location_lga;
    if (d.pickup_address !== undefined)
        updates["pickupAddress"] = d.pickup_address;
    if (d.description !== undefined)
        updates["description"] = d.description;
    if (d.harvest_date !== undefined)
        updates["harvestDate"] = d.harvest_date ? new Date(d.harvest_date) : null;
    if (d.delivery_options !== undefined)
        updates["deliveryOptions"] = d.delivery_options;
    ...
    if (d.quantity !== undefined)
        updates["quantity"] = String(d.quantity);
    if (d.price_per_unit !== undefined)
        updates["pricePerUnit"] = d.price_per_unit;
